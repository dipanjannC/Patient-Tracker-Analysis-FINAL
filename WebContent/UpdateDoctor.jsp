					...
					<div class="card-body">
						<div style="background: lavender">
							...
							<form:form action="updateDoctor" method="post"
								style="width: 100%;" modelAttribute="doctor">

								<%
									DoctorPOJO pojo = (DoctorPOJO) request.getAttribute("pojo");
								%>
								<form:input path="doctorId" type="text" name="doctorId"
									value="<%=pojo.getDoctorId()%>" readonly="true" />
								<hr>
								<form:input path="firstName" type="text" name="fname"
									value="<%=pojo.getFirstName()%>" maxlength="20"
									placeholder="First Name" required="required" />
								<hr>
								<form:input path="lastName" type="text" name="lname"
									value="<%=pojo.getLastName()%>" maxlength="20"
									placeholder="Last Name" required="required" />
								<hr>
								<form:input path="age" type="number" value="<%=pojo.getAge()%>"
									min="1" max="130" name="age" placeholder="Age" maxlength="3"
									required="required" />
								<hr>
								<form:select path="gender" id="gender" name="gender"
									value="<%=pojo.getGender()%>" class="form-control"
								...
								<form:input path="dateOfBirth" type="date" name="dob"
									value="<%=pojo.getDateOfBirth()%>"
									placeholder="Date of Birth (dd-mm-yyyy)" max="<%=fDate%>"
									required="required" />
								<hr>
								<label for="addressl1"></label>
								<form:input path="addressLine1" placeholder="Address Line 1"
									type="textarea" maxlength="20" name="addressl1" id=""
								...
								<form:input path="addressLine2" placeholder="Address Line 2"
									type="textarea" maxlength="20" name="addressl2"
									value="<%=pojo.getAddressLine2()%>" required="required" />
								<hr>
								<label for="degree"></label>
								<form:input path="degree" type="text" name="degree"
									placeholder="Degree" id="degree" maxlength="15"
									value="<%=pojo.getDegree()%>" required="required" />
								<hr>
								<label for="speciality"></label>
								<form:input path="speciality" type="text" name="speciality"
									placeholder="Speciality" id="speciality" maxlength="15"
									value="<%=pojo.getSpeciality()%>" required="required" />
								<hr>
								<label for="workhours"></label>
								<form:input path="workHours" type="number" name="workhours"
									placeholder="Work Hours" pattern="[0-9]{1,}"
									title="Don't begin with '0'" maxlength="2" id="workhours"
									value="<%=pojo.getWorkHours()%>" required="required" />
								<hr>
								...
								<form:input path="hospitalName" type="text" name="hospitalname"
									...
									value="<%=pojo.getHospitalName()%>" required="required" />
								<hr>
								<label for="city"></label>
								<form:input path="city" type="text" name="city"
									placeholder="City" id="city" maxlength="15"
									value="<%=pojo.getCity()%>" required="required" />
								<hr>
								<label for="state"></label>
								<form:input path="state" type="text" maxlength="15"
									placeholder="State" name="state" id="state"
									value="<%=pojo.getState()%>" required="required" />
								<hr>
								<label for="zipcode"></label>
								<form:input path="zipCode" type="text"
									placeholder="Enter Doctor's Zip Code" name="zipcode"
									id="zipcode" value="<%=pojo.getZipCode()%>" required="required"
									pattern="[0-9]{6,}"
								...
								<form:input path="emailId" type="email" name="email"
									maxlength="25" placeholder="Email ID" id="email"
									value="<%=pojo.getEmailId()%>" required="required" />
								<hr>
