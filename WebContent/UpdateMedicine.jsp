                ...
                <div class="card" style="box-shadow:0px 20px 50px grey;">
                    ...
                    <div class="card-body">
                        <div style="background:#f0e8f7">
                            ...
                            <form:form action="updateMedicine" method="post" style="width: 100%;" modelAttribute="medicine">
                            
                            	<%MedicinePOJO pojo = (MedicinePOJO)request.getAttribute("pojo"); %>
                            	
                            	<form:input path="medicineId" type="text" name="medicineId" value="<%=pojo.getMedicineId() %>" readonly="true" />
                                <hr>  
                                <form:input path="medicineDescription" type="textarea" name="description" placeholder="Medicine Description" value="<%=pojo.getMedicineDescription() %>" required="required" />
                                <hr>
                                <form:input path="cureFor" type="textarea" name="curefor" placeholder="Cure For" value="<%=pojo.getCureFor() %>" required="required" />
                                <hr>
                                <form:input path="manufacturingCompany" type="text" placeholder="Manufacturing Company" value="<%=pojo.getManufacturingCompany() %>" name="company" required="required"/>
                                <hr>
                                <form:input path="dosage" type="number" name="dosage" value="<%=pojo.getDosage() %>" placeholder="Enter Dosage Required" required="required" />
                                <hr>
                                <form:input path="prescribedFor" type="textarea" name="prescribedfor" value="<%=pojo.getPrescribedFor() %>" placeholder="Medicine Prescribed For"
                                    required="required" />
                                <hr>
                                <form:input path="amount" type="number" name="amount" value="<%=pojo.getAmount() %>" placeholder="Enter Medicine Amount" required="required"/>
                                <hr>
                                <div align="right">
                                    <input type="submit" class="btn btn-project" value="UPDATE" name="update"> <input type="reset" class="btn btn-danger" value="Cancel">
                                </div>
                            </form:form>
                        </div>
                    </div>
